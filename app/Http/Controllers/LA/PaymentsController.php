...
class PaymentsController extends Controller
{
	...
	public $view_col = 'iban_number';
	public $listing_cols = ['id', 'owner_name', 'iban_number', 'user_id', 'month', 'address_id', 'amount', 'payment_date'];
	 ...
	 */
        ...
        if ($list == 1) {
            $owners = DB::table('users')->where("role_id","=","11")->lists("name","id");
			...
			for ($j=0; $j < count($this->listing_cols); $j++) {
				...
				if($fields_popup[$col] != null && starts_with($fields_popup[$col]->popup_vals, "@")) {
					$data->data[$i][$j] = ModuleFields::getFieldValue($fields_popup[$col], $data->data[$i][$j]);
                                ...
                                if($col=="owner_name") {
                                    $owner = DB::table('users')->where('id',$data->data[$i][$j])->first();
                                    $data->data[$i][$j] = $owner->name;
                                }elseif($col == $this->view_col) {
