			...
			<div class="kt-login-v1__wrapper">
				<div class="kt-login-v1__container">
					<h3 class="kt-login-v1__title">
                                             <?php if(isset($user_details->email)) { ?>
							Update Password
                                                    <?php } else {  ?>
							Reset Password
                                                    <?php } ?>
					...
					<!--begin::Form-->
                            ...
                            @endif
					<form class="kt-login-v1__form kt-form" action="{{ url('/signup/change_password') }}" method="post" autocomplete="off">
						...
						<div class="form-group" style="{{ $display }}">
							<input class="form-control" type="email" placeholder="Email" name="email" autocomplete="off" value="<?php echo isset($user_details->email)?$user_details->email:''; ?>" <?php echo isset($user_details->email)?"readonly":''; ?>>
						</div>
						...
						<div class="kt-login-v1__actions">
							<a href="{{ url('/signin') }}" class="kt-login-v1__forgot">
								<i class="fa fa fa-long-arrow-left mr-10"></i> Back to Sign in
                                                    ...
                                                    <?php if(isset($user_details->email)) { ?>
							<button type="submit" class="btn btn-pill btn-elevate">Update Password</button>
                                                    <?php } else {  ?>
							<button type="submit" class="btn btn-pill btn-elevate">Reset Password</button>
                                                    <?php } ?>
						</div>
					</form>
					<!--end::Form-->
